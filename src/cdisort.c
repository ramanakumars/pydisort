	      ...
	      disort_output *out)
{
  ...
  if (ds->flag.intensity_correction &&
      (ds->flag.onlyfl || ds->bc.fbeam == 0. || !scat_yes || !deltam)) {
    corint = FALSE;
  }
  else {
    corint = TRUE;
  ...
  for (mazim = 0; mazim <= naz; mazim++) {
	...
	if ( ds->flag.spher == TRUE ) {
      ...
      if (!ds->flag.onlyfl && ds->flag.usrang) {
        ...
        c_interp_source(ds,lc,cwt,delm0,gl,mazim,oprim,ylm0,ylmc,ylmu,
			psi,xr,zee,zj,zbeam,zbu,zbs,zbsa,zu);
      }
    }
    ...
    c_solve0(ds,b,bdr,bem,bplanck,cband,cmu,cwt,expbea,ipvt,ll,lyrcut,
	     mazim,ncol,ncut,nn,tplanck,taucpr,z,zbeamsp,zbeama,zz,plk);
    ...
    if (mazim == 0) {
      c_fluxes(ds,out,ch,cmu,cwt,gc,kk,layru,ll,lyrcut,ncut,nn,PRNTU0(1),
	       taucpr,utaupr,xr,zbeamsp,zbeama,zz,plk,fl,u0c);
    }
...
void c_solve0_old(disort_state *ds,
		  double       *b,
		  double       *bdr,
		  double       *bem,
		  double        bplanck,
		  ...
		  disort_pair  *plk)
{
  ...
  /*
   * Construct B, STWJ(20a,c) for parallel beam+bottom reflection+thermal emission at top and/or bottom
   */
  if (mazim > 0 && ds->bc.fbeam > 0.) {
    /*
     * Azimuth-dependent case (never called if FBEAM = 0)
     */
    if (lyrcut || ds->flag.lamber) {
      /*
       * No azimuthal-dependent intensity for Lambert surface; no intensity component for truncated bottom layer
       */
      for (iq = 1; iq <= nn; iq++) {
        /*
         * Top boundary
    ...
    if (ds->bc.fbeam == 0.) {
      ...
      if (lyrcut) {
        for (iq = 1; iq <= nn; iq++) {
          B(ncol-nn+iq) = -ZZ(iq+nn,ncut)*EXPBEA(ncut)-ZPLK0(iq+nn,ncut)-ZPLK1(iq+nn,ncut)*TAUCPR(ncut);
        }
      }
      else {
        for (iq = 1; iq <= nn; iq++) {
         sum = 0.;
          for (jq = 1; jq <= nn; jq++) {
            sum += CWT(jq)*CMU(jq)*BDR(iq,jq)*(ZZ(nn+1-jq,ncut)*EXPBEA(ncut)+ZPLK0(nn+1-jq,ncut)+ZPLK1(nn+1-jq,ncut)*TAUCPR(ncut));
          }
      ...
      for (lc = 1; lc <= ncut-1; lc++) {
        for (iq = 1; iq <= ds->nstr; iq++) {
          B(++it) = (ZZ(iq,lc+1)-ZZ(iq,lc))*EXPBEA(lc)+ZPLK0(iq,lc+1)-ZPLK0(iq,lc)+(ZPLK1(iq,lc+1)-ZPLK1(iq,lc))*TAUCPR(lc);
        }
      }
    }
  }
  ...
  /*
   * Find L-U (lower/upper triangular) decomposition of band matrix CBAND and test if it is nearly
   * singular (note: CBAND is destroyed) (CBAND is in LINPACK packed format)
   */
  rcond = 0.;
  ncd   = 3*nn-1;
  c_sgbco(cband,(9*(ds->nstr/2)-2),ncol,ncd,ncd,ipvt,&rcond,z);
  ...
  /*
   * Solve linear system with coeff matrix CBAND and R.H. side(s) B after CBAND has been L-U decomposed.
   * Solution is returned in B.
   */
  c_sgbsl(cband,(9*(ds->nstr/2)-2),ncol,ncd,ncd,ipvt,b,0);

  /*
   * Zero CBAND (it may contain 'foreign' elements upon returning from LINPACK); necessary to prevent errors
   */
  memset(cband,0,(9*(ds->nstr/2)-2)*(ds->nstr*ds->nlyr)*sizeof(double));
  ...
  for (lu = 1; lu <= ds->ntau; lu++) {
    ...
    for (iu = 1; iu <= ds->numu; iu++) {
      ...
      if (negumu && mazim == 0) {
        ...
        bndint = (bnddfu+bnddir+delm0*EMU(iu)*bplanck)*exp((UTAUPR(lu)-TAUCPR(ds->nlyr))/UMU(iu));
      }
...
/*
    ...
    c_sgbfa is usually called by c_sbgco, but it can be called
    ...
    Outputs:
        ...
        info    int,
                ...
                = k  if  u(k,k) == 0.  This is not an error
                     ...
                     called.  Use  rcond  in c_sbgco for a reliable
...
/*
    ...
    Inputs:
        abd     double(lda, n), the output from sbgco or sgbfa.
        lda     int, the leading dimension of the array abd.
        n       int, the order of the original matrix.
        ml      int, number of diagonals below the main diagonal.
        mu      int, number of diagonals above the main diagonal.
        ipvt    int(n), the pivot vector from sbgco or sgbfa.
     ...
     Error condition:
        ...
        called correctly and if c_sbgco has set rcond > 0.0
              ...
              double         radius)
{
  ...
  for (lc = 1; lc <= ds->nlyr; lc++) {
    if(SSALB(lc) > 1.-DBL_EPSILON) {
      SSALB(lc) = 1.-DBL_EPSILON;
    }
    TAUC(lc) = TAUC(lc-1)+DTAUC(lc);
  }
                           ...
                           double       *tauc)
{
  ...
  if (inperr && ds->flag.quiet==VERBOSE) {
